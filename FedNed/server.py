...
from public_utils.utils.utils import get_model_score
...
class Server(object):
    def __init__(self,
        ...
        #teacher model
        self.global_teacher_good = build_model(args)
        ...
        self.global_teacher_bad  = build_model(args)
        ...
        self.warm_round = args.warm_round
    ...
    def compute_uncertainty(self):
        ...
        logging.info('list_key_good:{}'.format(list_key_good))
        logging.info('list_key_bad:{}'.format(list_key_bad))
    ...
    def aggregate_distillation(self):
        ...
        for step in range(100):
            ...
            _, _, avg_logits_good_teacher = self.global_teacher_good(images_unlabeled,True,True)
    ...
    def train(self):
        ...
        for i in range(1, self.global_rounds + 1):
            ...
            if i <= self.warm_round:
                ...
                logging.info('selected_client_indexes: {}'.format(self.selected_client_indexes))
                self.send_models()
                for select_id in self.selected_client_indexes:
                    logging.info(f"第{select_id}客户端")
                ...
                self.list_dicts_local_params = []
                self.list_nums_local_data = []
                _, bacc = self.evaluate()
                self.BACC.append(bacc)
                # logging.info('Round: {}'.format(i))
                # logging.info('bess acc nowacc:{:.5f}'.format(acc))
                if best_performance < bacc:
                    best_performance = bacc
                logging.info(f'eopch:{i} best acc: {best_performance}, now acc: {bacc}')
            else:
                self.selected_client_indexes= self.select_clinet_indexes()
                logging.info('selected_client_indexes: {}'.format(self.selected_client_indexes))
                self.send_models()
                logging.info('EN indax: {}'.format(bad_log))
                for select_id in self.selected_client_indexes:
                    logging.info(f"第{select_id}客户端")
                ...
                logging.info('good_client: {}'.format(self.selected_good_client_indexes))
                logging.info('bad_client: {}'.format(self.selected_bad_client_indexes))
                ...
                self.BACC.append(bacc)
                # logging.info('Round: {}'.format(i))
                # logging.info('Acc:{:.5f}'.format(acc))
                if best_performance < bacc:
                    best_performance = bacc
                logging.info(f'eopch:{i} best acc: {best_performance}, now acc: {bacc}')
    ...
    def evaluate(self):
        self.global_teacher_good.eval()
        test_loader = DataLoader(dataset=self.global_test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=4)
        pred = np.array([])
        with torch.no_grad():
            for images, labels in test_loader:
                images = images.to(self.device)
                labels = labels.to(self.device)
                _, _, outputs = self.global_teacher_good(images,True,True)
                _, predicted = torch.max(outputs.data, 1)
                pred = np.concatenate([pred, predicted.detach().cpu().numpy()], axis=0)
            
        # 计算
        acc, bacc = get_model_score(self.global_test_dataset.targets,pred)
        return acc, bacc
