...
class Client(object):
    def __init__(self, args, data_client, train_sapmles, model):
        ...
        self.model_classifier = model.resnet.fc
        self.model_classifier.to(self.device)

        self.ce_loss = nn.CrossEntropyLoss()
        # self.optimizer = optim.SGD(self.model.parameters(), lr=self.learning_rate)
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.learning_rate, betas=(0.9, 0.999), weight_decay=5e-4)
    ...
    def train(self):
        ...
        for step in range(self.local_steps):
            ...
            for images, labels, indexs in data_loader:
                ...
                _, _, output = self.model(images,True,True)
                loss = self.ce_loss(output, labels)
                batch_loss.append(loss.item())
                loss.backward()
                self.optimizer.step()
            if step % 5 == 0 :
                logging.info("train client epoch = {} loss = {}".format(step,sum(batch_loss)/len(batch_loss)))
    ...
    def bad_train(self):
        ...
        for step in range(self.local_steps):
            ...
            for inputs, label, indexs in labeled_data_loader:
                ...
                self.optimizer.step()
            if step % 5 == 0 :
                logging.info("bad train client epoch = {} loss = {}".format(step,sum(batch_loss)/len(batch_loss)))
    ...
    def get_params(self):
        ...
        for step in range(self.local_steps):
            for images, labels in data_loader:
                images, labels = images.to(self.device), labels.to(self.device)
                self.optimizer.zero_grad()
                feature, _, _ = self.model(images,True,True)
                feature.detach()
