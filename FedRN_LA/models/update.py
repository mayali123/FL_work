...
from public_utils.utils.losses import LogitAdjust
from public_utils.utils.utils import get_num_of_each_class
...
def get_local_update_objects(args, dataset_train, dict_users=None, noise_rates=None, gaussian_noise=None,is_clean=None):
...
class BaseLocalUpdate:
    def __init__(
        ...
        self.args = args
        # 
        logging.info("使用LA 损失函数")
        # self.loss_func = nn.CrossEntropyLoss()
        self.class_num_list = get_num_of_each_class(dataset, idxs, self.args.n_classes)
...
class LocalUpdateFedRN(BaseLocalUpdate):
    def __init__(self, args, dataset=None, user_idx=None, idxs=None, gaussian_noise=None,is_clean=None):
        super().__init__(
            args=args,
            dataset=dataset,
            user_idx=user_idx,
    ...
    # pred_clean_idx 是dataset得idx
    def compute_detect_acc(self, pred_clean_idx):
        # pre_clean_acc = 0
        # pre_noisy_acc = 0
        # all_smaple_num = 0
        pred_info_dict = {}
        ...
        # 每个类别预测干净样本预测正确的数量
        class_pre_clean_acc = [0 for idx in range(self.args.n_classes)]
        # 每个类别预测噪声样本预测正确的数量
        class_pre_noisy_acc = [0 for idx in range(self.args.n_classes)]
        # 每个类别预测噪声样本预测正确的数量
        # class_smaple_num = [0 for idx in range(self.args.n_classes)]
        

        for sample in self.data_indices:
            cls = self.dataset.targets[sample]
            if sample in pred_clean_idx and self.is_clean[sample] == True:
                class_pre_clean_acc[cls] += 1
            if self.is_clean[sample] == False and sample not in pred_clean_idx:
                class_pre_noisy_acc[cls] += 1
     
        pred_info_dict['class_pre_clean_acc'] = np.array(class_pre_clean_acc)
        pred_info_dict['class_pre_noisy_acc'] = np.array(class_pre_noisy_acc)
        pred_info_dict['class_smaple_num'] = np.array(self.class_num_list)

        logging.info(f"pre_clean_acc:{sum(class_pre_clean_acc)},pre_noisy_acc:{sum(class_pre_noisy_acc)},all_smaple_num:{sum(self.class_num_list)}")
        logging.info(f"detect_acc = :{(sum(class_pre_clean_acc) + sum(class_pre_noisy_acc))/sum(self.class_num_list) }")

        
        return pred_info_dict

    def train_phase2_with_global_net_detect(self, g_net):
        g_prob = self.fit_gmm(g_net)
        g_clean_idx, g_noisy_idx = self.get_clean_idx(g_prob)
        detect_info_dict = self.compute_detect_acc(g_clean_idx)

        
        # Update loader with final clean idxs
        self.ldr_train = DataLoader(DatasetSplit(self.dataset, g_clean_idx, real_idx_return=True),
                                    batch_size=self.args.local_bs,
                                    shuffle=True,
                                    num_workers=self.args.num_workers,
                                    pin_memory=True,
        ...
        # local training
        w, loss = self.train_single_model(g_net)
        self.set_expertise()
        self.set_arbitrary_output()
        return w, loss, detect_info_dict
